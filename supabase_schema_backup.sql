


SET statement_timeout = 0;
SET lock_timeout = 0;
SET idle_in_transaction_session_timeout = 0;
SET client_encoding = 'UTF8';
SET standard_conforming_strings = on;
SELECT pg_catalog.set_config('search_path', '', false);
SET check_function_bodies = false;
SET xmloption = content;
SET client_min_messages = warning;
SET row_security = off;


COMMENT ON SCHEMA "public" IS 'standard public schema';



CREATE EXTENSION IF NOT EXISTS "moddatetime" WITH SCHEMA "extensions";






CREATE EXTENSION IF NOT EXISTS "pg_graphql" WITH SCHEMA "graphql";






CREATE EXTENSION IF NOT EXISTS "pg_stat_statements" WITH SCHEMA "extensions";






CREATE EXTENSION IF NOT EXISTS "pgcrypto" WITH SCHEMA "extensions";






CREATE EXTENSION IF NOT EXISTS "supabase_vault" WITH SCHEMA "vault";






CREATE EXTENSION IF NOT EXISTS "uuid-ossp" WITH SCHEMA "extensions";






CREATE TYPE "public"."ai_job_status" AS ENUM (
    'idle',
    'queued',
    'processing',
    'completed',
    'error'
);


ALTER TYPE "public"."ai_job_status" OWNER TO "postgres";


CREATE TYPE "public"."summary_status" AS ENUM (
    'pending',
    'processing',
    'completed',
    'failed'
);


ALTER TYPE "public"."summary_status" OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_sync_braindump_title_to_kanban"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    IF (NEW.title IS DISTINCT FROM OLD.title) THEN
        UPDATE public.tasks 
        SET title = NEW.title 
        WHERE linked_board_id = NEW.id 
          AND (title IS DISTINCT FROM NEW.title);
    END IF;
    RETURN NEW;
END;
$$;


ALTER FUNCTION "public"."fn_sync_braindump_title_to_kanban"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_sync_braindump_title_to_tasks"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    IF (NEW.title IS DISTINCT FROM OLD.title) THEN
        UPDATE public.tasks 
        SET title = NEW.title 
        WHERE (linked_board_id = NEW.id OR id = NEW.id)
          AND (title IS DISTINCT FROM NEW.title);
    END IF;
    RETURN NEW;
END;
$$;


ALTER FUNCTION "public"."fn_sync_braindump_title_to_tasks"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_sync_note_title_to_tasks"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    IF (NEW.title IS DISTINCT FROM OLD.title) THEN
        UPDATE public.tasks 
        SET title = NEW.title 
        WHERE (linked_note_id = NEW.id OR id = NEW.id)
          AND (title IS DISTINCT FROM NEW.title);
    END IF;
    RETURN NEW;
END;
$$;


ALTER FUNCTION "public"."fn_sync_note_title_to_tasks"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_sync_task_title_to_braindump"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    IF (NEW.title IS DISTINCT FROM OLD.title AND NEW.linked_board_id IS NOT NULL) THEN
        UPDATE public.brain_dumps 
        SET title = NEW.title 
        WHERE id = NEW.linked_board_id 
          AND (title IS DISTINCT FROM NEW.title);
    END IF;
    RETURN NEW;
END;
$$;


ALTER FUNCTION "public"."fn_sync_task_title_to_braindump"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_sync_task_title_to_links"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    IF (NEW.title IS DISTINCT FROM OLD.title) THEN
        -- Sincronizar hacia Notas (por vínculo o por ID legacy)
        UPDATE public.notes 
        SET title = NEW.title 
        WHERE (id = NEW.linked_note_id OR id = NEW.id) 
          AND (title IS DISTINCT FROM NEW.title);

        -- Sincronizar hacia Pizarrones (por vínculo o por ID legacy)
        UPDATE public.brain_dumps 
        SET title = NEW.title 
        WHERE (id = NEW.linked_board_id OR id = NEW.id) 
          AND (title IS DISTINCT FROM NEW.title);
    END IF;
    RETURN NEW;
END;
$$;


ALTER FUNCTION "public"."fn_sync_task_title_to_links"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."update_modified_column"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    NEW.updated_at = now();
    RETURN NEW;   
END;
$$;


ALTER FUNCTION "public"."update_modified_column"() OWNER TO "postgres";

SET default_tablespace = '';

SET default_table_access_method = "heap";


CREATE TABLE IF NOT EXISTS "public"."brain_dumps" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_id" "uuid" DEFAULT "auth"."uid"() NOT NULL,
    "content" "text" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"(),
    "status" character varying DEFAULT 'history'::character varying,
    "updated_at" timestamp with time zone DEFAULT "now"(),
    "title" "text",
    "is_checklist" boolean DEFAULT false,
    "ai_summary_status" "public"."ai_job_status" DEFAULT 'idle'::"public"."ai_job_status",
    "focus_prompt" "text",
    "generation_level" integer DEFAULT 0,
    "generation_status" "text" DEFAULT 'idle'::"text",
    "parent_id" "uuid",
    "scratchpad" "text",
    "tiktok_video_id" "uuid"
);


ALTER TABLE "public"."brain_dumps" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."groups" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_id" "uuid" NOT NULL,
    "name" "text" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "timezone"('utc'::"text", "now"()) NOT NULL,
    "is_pinned" boolean DEFAULT false,
    "last_accessed_at" timestamp with time zone DEFAULT "now"(),
    "is_favorite" boolean DEFAULT false
);


ALTER TABLE "public"."groups" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."notes" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_id" "uuid" NOT NULL,
    "group_id" "uuid",
    "title" "text",
    "content" "text",
    "position" integer DEFAULT 0,
    "created_at" timestamp with time zone DEFAULT "timezone"('utc'::"text", "now"()) NOT NULL,
    "is_pinned" boolean DEFAULT false,
    "updated_at" timestamp with time zone DEFAULT "now"(),
    "is_docked" boolean DEFAULT false,
    "is_checklist" boolean DEFAULT false,
    "ai_summary_status" "public"."ai_job_status" DEFAULT 'idle'::"public"."ai_job_status",
    "ai_generated" boolean DEFAULT false,
    "focus_prompt" "text",
    "generation_level" integer DEFAULT 0,
    "generation_status" "text" DEFAULT 'idle'::"text",
    "parent_note_id" "uuid",
    "scratchpad" "text" DEFAULT ''::"text",
    "is_open" boolean DEFAULT false,
    "tiktok_video_id" "uuid",
    "status" "text" DEFAULT 'main'::"text" NOT NULL
);


ALTER TABLE "public"."notes" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."reminders" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "title" "text" NOT NULL,
    "content" "text",
    "due_at" timestamp with time zone,
    "is_completed" boolean DEFAULT false NOT NULL,
    "user_id" "uuid" DEFAULT "auth"."uid"() NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"(),
    "status" "text" DEFAULT 'active'::"text",
    "targets" "jsonb" DEFAULT '[]'::"jsonb",
    "updated_at" timestamp with time zone DEFAULT "timezone"('utc'::"text", "now"())
);


ALTER TABLE "public"."reminders" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."summaries" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "note_id" "uuid",
    "target_objective" "text",
    "content" "text",
    "status" "public"."summary_status" DEFAULT 'pending'::"public"."summary_status",
    "created_at" timestamp with time zone DEFAULT "now"(),
    "brain_dump_id" "uuid",
    "scratchpad" "text" DEFAULT ''::"text",
    "tiktok_video_id" "uuid"
);


ALTER TABLE "public"."summaries" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."tasks" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "title" "text" NOT NULL,
    "status" "text" DEFAULT 'backlog'::"text" NOT NULL,
    "position" integer DEFAULT 0 NOT NULL,
    "user_id" "uuid" DEFAULT "auth"."uid"() NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"(),
    "updated_at" timestamp with time zone DEFAULT "now"(),
    "source_id" "uuid",
    "content" "text",
    "linked_note_id" "uuid",
    "linked_board_id" "uuid"
);


ALTER TABLE "public"."tasks" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."tiktok_note_links" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "video_id" "uuid" NOT NULL,
    "note_id" "uuid",
    "group_id" "uuid",
    "user_id" "uuid" DEFAULT "auth"."uid"() NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"()
);


ALTER TABLE "public"."tiktok_note_links" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."tiktok_queue" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_id" "uuid" DEFAULT "auth"."uid"() NOT NULL,
    "url" "text" NOT NULL,
    "status" "text" DEFAULT 'pending'::"text" NOT NULL,
    "error_msg" "text",
    "video_id" "uuid",
    "created_at" timestamp with time zone DEFAULT "now"(),
    "updated_at" timestamp with time zone DEFAULT "now"(),
    "parent_id" "uuid"
);


ALTER TABLE "public"."tiktok_queue" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."tiktok_videos" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_id" "uuid" DEFAULT "auth"."uid"() NOT NULL,
    "url" "text" NOT NULL,
    "title" "text",
    "author" "text",
    "duration" integer,
    "thumbnail" "text",
    "view_count" integer DEFAULT 0,
    "like_count" integer DEFAULT 0,
    "transcript" "text",
    "summary" "text",
    "key_points" "jsonb" DEFAULT '[]'::"jsonb",
    "category" "text",
    "language" "text",
    "created_at" timestamp with time zone DEFAULT "now"(),
    "updated_at" timestamp with time zone DEFAULT "now"(),
    "content" "text" DEFAULT ''::"text",
    "scratchpad" "text" DEFAULT ''::"text",
    "status" "text" DEFAULT 'inbox'::"text",
    "ai_summary_status" "text" DEFAULT 'idle'::"text",
    "description" "text",
    "parent_id" "uuid",
    "generation_level" integer DEFAULT 0
);


ALTER TABLE "public"."tiktok_videos" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."timer_logs" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "timer_id" "uuid" NOT NULL,
    "user_id" "uuid" DEFAULT "auth"."uid"() NOT NULL,
    "duration_seconds" integer NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"()
);


ALTER TABLE "public"."timer_logs" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."timers" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "title" "text" NOT NULL,
    "type" "text" DEFAULT 'stopwatch'::"text" NOT NULL,
    "status" "text" DEFAULT 'paused'::"text" NOT NULL,
    "accumulated_seconds" integer DEFAULT 0 NOT NULL,
    "target_seconds" integer DEFAULT 0,
    "last_started_at" timestamp with time zone,
    "user_id" "uuid" DEFAULT "auth"."uid"() NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"(),
    "updated_at" timestamp with time zone DEFAULT "now"(),
    "content" "text" DEFAULT ''::"text",
    "laps" "jsonb" DEFAULT '[]'::"jsonb",
    "accumulated_ms" bigint DEFAULT 0
);


ALTER TABLE "public"."timers" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."translations" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_id" "uuid" DEFAULT "auth"."uid"() NOT NULL,
    "source_text" "text" NOT NULL,
    "translated_text" "text" NOT NULL,
    "source_lang" character varying(10) NOT NULL,
    "target_lang" character varying(10) NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"()
);


ALTER TABLE "public"."translations" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."yo_memoria" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_id" "uuid" NOT NULL,
    "temas" "text"[] DEFAULT '{}'::"text"[],
    "marcadores_frecuentes" "jsonb" DEFAULT '[]'::"jsonb",
    "patrones" "text"[] DEFAULT '{}'::"text"[],
    "preguntas_abiertas" "text"[] DEFAULT '{}'::"text"[],
    "palabras_clave" "text"[] DEFAULT '{}'::"text"[],
    "total_notas_procesadas" integer DEFAULT 0,
    "updated_at" timestamp with time zone DEFAULT "now"()
);


ALTER TABLE "public"."yo_memoria" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."yo_perfil" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "user_id" "uuid" NOT NULL,
    "yo_json" "jsonb" DEFAULT '{}'::"jsonb",
    "updated_at" timestamp with time zone DEFAULT "now"()
);


ALTER TABLE "public"."yo_perfil" OWNER TO "postgres";


ALTER TABLE ONLY "public"."brain_dumps"
    ADD CONSTRAINT "brain_dumps_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."groups"
    ADD CONSTRAINT "groups_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."notes"
    ADD CONSTRAINT "notes_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."reminders"
    ADD CONSTRAINT "reminders_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."summaries"
    ADD CONSTRAINT "summaries_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."tasks"
    ADD CONSTRAINT "tasks_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."tiktok_note_links"
    ADD CONSTRAINT "tiktok_note_links_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."tiktok_queue"
    ADD CONSTRAINT "tiktok_queue_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."tiktok_videos"
    ADD CONSTRAINT "tiktok_videos_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."timer_logs"
    ADD CONSTRAINT "timer_logs_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."timers"
    ADD CONSTRAINT "timers_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."translations"
    ADD CONSTRAINT "translations_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."yo_memoria"
    ADD CONSTRAINT "yo_memoria_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."yo_perfil"
    ADD CONSTRAINT "yo_perfil_pkey" PRIMARY KEY ("id");



CREATE INDEX "idx_brain_dumps_parent" ON "public"."brain_dumps" USING "btree" ("parent_id");



CREATE INDEX "idx_brain_dumps_tiktok_video_id" ON "public"."brain_dumps" USING "btree" ("tiktok_video_id");



CREATE INDEX "idx_notes_ai_status" ON "public"."notes" USING "btree" ("ai_summary_status") WHERE ("ai_summary_status" = 'queued'::"public"."ai_job_status");



CREATE INDEX "idx_notes_parent" ON "public"."notes" USING "btree" ("parent_note_id");



CREATE INDEX "idx_notes_status" ON "public"."notes" USING "btree" ("status");



CREATE INDEX "idx_notes_tiktok_video_id" ON "public"."notes" USING "btree" ("tiktok_video_id");



CREATE INDEX "idx_summaries_brain_dump_id" ON "public"."summaries" USING "btree" ("brain_dump_id");



CREATE INDEX "idx_summaries_note_id" ON "public"."summaries" USING "btree" ("note_id");



CREATE INDEX "idx_summaries_tiktok_video_id" ON "public"."summaries" USING "btree" ("tiktok_video_id");



CREATE INDEX "idx_tasks_source_id" ON "public"."tasks" USING "btree" ("source_id");



CREATE INDEX "idx_tiktok_queue_parent" ON "public"."tiktok_queue" USING "btree" ("parent_id");



CREATE INDEX "idx_tiktok_queue_status" ON "public"."tiktok_queue" USING "btree" ("status", "created_at");



CREATE INDEX "idx_tiktok_queue_user" ON "public"."tiktok_queue" USING "btree" ("user_id");



CREATE INDEX "idx_tiktok_videos_parent" ON "public"."tiktok_videos" USING "btree" ("parent_id");



CREATE INDEX "idx_tiktok_videos_status" ON "public"."tiktok_videos" USING "btree" ("status");



CREATE INDEX "idx_tiktok_videos_user" ON "public"."tiktok_videos" USING "btree" ("user_id", "created_at" DESC);



CREATE OR REPLACE TRIGGER "handle_tasks_updated_at" BEFORE UPDATE ON "public"."tasks" FOR EACH ROW EXECUTE FUNCTION "extensions"."moddatetime"('updated_at');



CREATE OR REPLACE TRIGGER "handle_timers_updated_at" BEFORE UPDATE ON "public"."timers" FOR EACH ROW EXECUTE FUNCTION "extensions"."moddatetime"('updated_at');



CREATE OR REPLACE TRIGGER "handle_updated_at" BEFORE UPDATE ON "public"."notes" FOR EACH ROW EXECUTE FUNCTION "extensions"."moddatetime"('updated_at');



CREATE OR REPLACE TRIGGER "tr_sync_braindump_primary_title" AFTER UPDATE OF "title" ON "public"."brain_dumps" FOR EACH ROW EXECUTE FUNCTION "public"."fn_sync_braindump_title_to_tasks"();



CREATE OR REPLACE TRIGGER "tr_sync_braindump_title" AFTER UPDATE OF "title" ON "public"."brain_dumps" FOR EACH ROW EXECUTE FUNCTION "public"."fn_sync_braindump_title_to_tasks"();



CREATE OR REPLACE TRIGGER "tr_sync_braindump_to_task" AFTER UPDATE OF "title" ON "public"."brain_dumps" FOR EACH ROW EXECUTE FUNCTION "public"."fn_sync_braindump_title_to_kanban"();



CREATE OR REPLACE TRIGGER "tr_sync_note_title" AFTER UPDATE OF "title" ON "public"."notes" FOR EACH ROW EXECUTE FUNCTION "public"."fn_sync_note_title_to_tasks"();



CREATE OR REPLACE TRIGGER "tr_sync_task_title" AFTER UPDATE OF "title" ON "public"."tasks" FOR EACH ROW EXECUTE FUNCTION "public"."fn_sync_task_title_to_links"();



CREATE OR REPLACE TRIGGER "tr_sync_task_to_braindump" AFTER UPDATE OF "title" ON "public"."tasks" FOR EACH ROW EXECUTE FUNCTION "public"."fn_sync_task_title_to_braindump"();



CREATE OR REPLACE TRIGGER "trg_tiktok_queue_updated" BEFORE UPDATE ON "public"."tiktok_queue" FOR EACH ROW EXECUTE FUNCTION "public"."update_modified_column"();



CREATE OR REPLACE TRIGGER "trg_tiktok_videos_updated" BEFORE UPDATE ON "public"."tiktok_videos" FOR EACH ROW EXECUTE FUNCTION "public"."update_modified_column"();



CREATE OR REPLACE TRIGGER "update_reminders_modtime" BEFORE UPDATE ON "public"."reminders" FOR EACH ROW EXECUTE FUNCTION "public"."update_modified_column"();



ALTER TABLE ONLY "public"."brain_dumps"
    ADD CONSTRAINT "brain_dumps_parent_id_fkey" FOREIGN KEY ("parent_id") REFERENCES "public"."brain_dumps"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."brain_dumps"
    ADD CONSTRAINT "brain_dumps_tiktok_video_id_fkey" FOREIGN KEY ("tiktok_video_id") REFERENCES "public"."tiktok_videos"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."brain_dumps"
    ADD CONSTRAINT "brain_dumps_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."tiktok_queue"
    ADD CONSTRAINT "fk_tiktok_queue_video" FOREIGN KEY ("video_id") REFERENCES "public"."tiktok_videos"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."groups"
    ADD CONSTRAINT "groups_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id");



ALTER TABLE ONLY "public"."notes"
    ADD CONSTRAINT "notes_group_id_fkey" FOREIGN KEY ("group_id") REFERENCES "public"."groups"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."notes"
    ADD CONSTRAINT "notes_parent_note_id_fkey" FOREIGN KEY ("parent_note_id") REFERENCES "public"."notes"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."notes"
    ADD CONSTRAINT "notes_tiktok_video_id_fkey" FOREIGN KEY ("tiktok_video_id") REFERENCES "public"."tiktok_videos"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."notes"
    ADD CONSTRAINT "notes_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id");



ALTER TABLE ONLY "public"."reminders"
    ADD CONSTRAINT "reminders_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."summaries"
    ADD CONSTRAINT "summaries_brain_dump_id_fkey" FOREIGN KEY ("brain_dump_id") REFERENCES "public"."brain_dumps"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."summaries"
    ADD CONSTRAINT "summaries_note_id_fkey" FOREIGN KEY ("note_id") REFERENCES "public"."notes"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."summaries"
    ADD CONSTRAINT "summaries_tiktok_video_id_fkey" FOREIGN KEY ("tiktok_video_id") REFERENCES "public"."tiktok_videos"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."tasks"
    ADD CONSTRAINT "tasks_linked_board_id_fkey" FOREIGN KEY ("linked_board_id") REFERENCES "public"."brain_dumps"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."tasks"
    ADD CONSTRAINT "tasks_linked_note_id_fkey" FOREIGN KEY ("linked_note_id") REFERENCES "public"."notes"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."tasks"
    ADD CONSTRAINT "tasks_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."tiktok_note_links"
    ADD CONSTRAINT "tiktok_note_links_group_id_fkey" FOREIGN KEY ("group_id") REFERENCES "public"."groups"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."tiktok_note_links"
    ADD CONSTRAINT "tiktok_note_links_note_id_fkey" FOREIGN KEY ("note_id") REFERENCES "public"."notes"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."tiktok_note_links"
    ADD CONSTRAINT "tiktok_note_links_video_id_fkey" FOREIGN KEY ("video_id") REFERENCES "public"."tiktok_videos"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."tiktok_queue"
    ADD CONSTRAINT "tiktok_queue_parent_id_fkey" FOREIGN KEY ("parent_id") REFERENCES "public"."tiktok_videos"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."tiktok_videos"
    ADD CONSTRAINT "tiktok_videos_parent_id_fkey" FOREIGN KEY ("parent_id") REFERENCES "public"."tiktok_videos"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."timer_logs"
    ADD CONSTRAINT "timer_logs_timer_id_fkey" FOREIGN KEY ("timer_id") REFERENCES "public"."timers"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."timer_logs"
    ADD CONSTRAINT "timer_logs_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."timers"
    ADD CONSTRAINT "timers_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."translations"
    ADD CONSTRAINT "translations_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."yo_memoria"
    ADD CONSTRAINT "yo_memoria_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."yo_perfil"
    ADD CONSTRAINT "yo_perfil_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



CREATE POLICY "Users can manage summaries of their notes" ON "public"."summaries" TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."notes"
  WHERE (("notes"."id" = "summaries"."note_id") AND ("notes"."user_id" = "auth"."uid"())))));



CREATE POLICY "Users can manage summaries of their own records" ON "public"."summaries" TO "authenticated" USING (((EXISTS ( SELECT 1
   FROM "public"."notes"
  WHERE (("notes"."id" = "summaries"."note_id") AND ("notes"."user_id" = "auth"."uid"())))) OR (EXISTS ( SELECT 1
   FROM "public"."brain_dumps"
  WHERE (("brain_dumps"."id" = "summaries"."brain_dump_id") AND ("brain_dumps"."user_id" = "auth"."uid"())))) OR (EXISTS ( SELECT 1
   FROM "public"."tiktok_videos"
  WHERE (("tiktok_videos"."id" = "summaries"."tiktok_video_id") AND ("tiktok_videos"."user_id" = "auth"."uid"()))))));



CREATE POLICY "Users can manage their own brain_dumps" ON "public"."brain_dumps" TO "authenticated" USING (("user_id" = "auth"."uid"())) WITH CHECK (("user_id" = "auth"."uid"()));



CREATE POLICY "Users can manage their own reminders" ON "public"."reminders" USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Users can manage their own tasks" ON "public"."tasks" USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Users can manage their own timer logs" ON "public"."timer_logs" USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Users can manage their own timers" ON "public"."timers" USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Users manage their own dumps" ON "public"."brain_dumps" USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Users manage their own translations" ON "public"."translations" USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Usuarios pueden actualizar sus grupos" ON "public"."groups" FOR UPDATE USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Usuarios pueden actualizar sus notas" ON "public"."notes" FOR UPDATE USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Usuarios pueden borrar sus grupos" ON "public"."groups" FOR DELETE USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Usuarios pueden borrar sus notas" ON "public"."notes" FOR DELETE USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Usuarios pueden crear grupos" ON "public"."groups" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "Usuarios pueden crear notas" ON "public"."notes" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "Usuarios pueden ver sus propias notas" ON "public"."notes" FOR SELECT USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Usuarios pueden ver sus propios grupos" ON "public"."groups" FOR SELECT USING (("auth"."uid"() = "user_id"));



ALTER TABLE "public"."brain_dumps" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."groups" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."notes" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."reminders" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."summaries" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."tasks" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "tiktok_links_owner" ON "public"."tiktok_note_links" USING (("user_id" = "auth"."uid"()));



ALTER TABLE "public"."tiktok_note_links" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."tiktok_queue" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "tiktok_queue_owner" ON "public"."tiktok_queue" USING (("user_id" = "auth"."uid"()));



ALTER TABLE "public"."tiktok_videos" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "tiktok_videos_owner" ON "public"."tiktok_videos" USING (("user_id" = "auth"."uid"()));



ALTER TABLE "public"."timer_logs" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."timers" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."translations" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "usuario ve su memoria" ON "public"."yo_memoria" USING (("auth"."uid"() = "user_id"));



CREATE POLICY "usuario ve su perfil" ON "public"."yo_perfil" USING (("auth"."uid"() = "user_id"));



ALTER TABLE "public"."yo_memoria" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."yo_perfil" ENABLE ROW LEVEL SECURITY;




ALTER PUBLICATION "supabase_realtime" OWNER TO "postgres";






ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."brain_dumps";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."groups";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."notes";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."reminders";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."summaries";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."tasks";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."tiktok_queue";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."tiktok_videos";



ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "public"."translations";



GRANT USAGE ON SCHEMA "public" TO "postgres";
GRANT USAGE ON SCHEMA "public" TO "anon";
GRANT USAGE ON SCHEMA "public" TO "authenticated";
GRANT USAGE ON SCHEMA "public" TO "service_role";




























































































































































GRANT ALL ON FUNCTION "public"."fn_sync_braindump_title_to_kanban"() TO "anon";
GRANT ALL ON FUNCTION "public"."fn_sync_braindump_title_to_kanban"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_sync_braindump_title_to_kanban"() TO "service_role";



GRANT ALL ON FUNCTION "public"."fn_sync_braindump_title_to_tasks"() TO "anon";
GRANT ALL ON FUNCTION "public"."fn_sync_braindump_title_to_tasks"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_sync_braindump_title_to_tasks"() TO "service_role";



GRANT ALL ON FUNCTION "public"."fn_sync_note_title_to_tasks"() TO "anon";
GRANT ALL ON FUNCTION "public"."fn_sync_note_title_to_tasks"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_sync_note_title_to_tasks"() TO "service_role";



GRANT ALL ON FUNCTION "public"."fn_sync_task_title_to_braindump"() TO "anon";
GRANT ALL ON FUNCTION "public"."fn_sync_task_title_to_braindump"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_sync_task_title_to_braindump"() TO "service_role";



GRANT ALL ON FUNCTION "public"."fn_sync_task_title_to_links"() TO "anon";
GRANT ALL ON FUNCTION "public"."fn_sync_task_title_to_links"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_sync_task_title_to_links"() TO "service_role";



GRANT ALL ON FUNCTION "public"."update_modified_column"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_modified_column"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_modified_column"() TO "service_role";


















GRANT ALL ON TABLE "public"."brain_dumps" TO "anon";
GRANT ALL ON TABLE "public"."brain_dumps" TO "authenticated";
GRANT ALL ON TABLE "public"."brain_dumps" TO "service_role";



GRANT ALL ON TABLE "public"."groups" TO "anon";
GRANT ALL ON TABLE "public"."groups" TO "authenticated";
GRANT ALL ON TABLE "public"."groups" TO "service_role";



GRANT ALL ON TABLE "public"."notes" TO "anon";
GRANT ALL ON TABLE "public"."notes" TO "authenticated";
GRANT ALL ON TABLE "public"."notes" TO "service_role";



GRANT ALL ON TABLE "public"."reminders" TO "anon";
GRANT ALL ON TABLE "public"."reminders" TO "authenticated";
GRANT ALL ON TABLE "public"."reminders" TO "service_role";



GRANT ALL ON TABLE "public"."summaries" TO "anon";
GRANT ALL ON TABLE "public"."summaries" TO "authenticated";
GRANT ALL ON TABLE "public"."summaries" TO "service_role";



GRANT ALL ON TABLE "public"."tasks" TO "anon";
GRANT ALL ON TABLE "public"."tasks" TO "authenticated";
GRANT ALL ON TABLE "public"."tasks" TO "service_role";



GRANT ALL ON TABLE "public"."tiktok_note_links" TO "anon";
GRANT ALL ON TABLE "public"."tiktok_note_links" TO "authenticated";
GRANT ALL ON TABLE "public"."tiktok_note_links" TO "service_role";



GRANT ALL ON TABLE "public"."tiktok_queue" TO "anon";
GRANT ALL ON TABLE "public"."tiktok_queue" TO "authenticated";
GRANT ALL ON TABLE "public"."tiktok_queue" TO "service_role";



GRANT ALL ON TABLE "public"."tiktok_videos" TO "anon";
GRANT ALL ON TABLE "public"."tiktok_videos" TO "authenticated";
GRANT ALL ON TABLE "public"."tiktok_videos" TO "service_role";



GRANT ALL ON TABLE "public"."timer_logs" TO "anon";
GRANT ALL ON TABLE "public"."timer_logs" TO "authenticated";
GRANT ALL ON TABLE "public"."timer_logs" TO "service_role";



GRANT ALL ON TABLE "public"."timers" TO "anon";
GRANT ALL ON TABLE "public"."timers" TO "authenticated";
GRANT ALL ON TABLE "public"."timers" TO "service_role";



GRANT ALL ON TABLE "public"."translations" TO "anon";
GRANT ALL ON TABLE "public"."translations" TO "authenticated";
GRANT ALL ON TABLE "public"."translations" TO "service_role";



GRANT ALL ON TABLE "public"."yo_memoria" TO "anon";
GRANT ALL ON TABLE "public"."yo_memoria" TO "authenticated";
GRANT ALL ON TABLE "public"."yo_memoria" TO "service_role";



GRANT ALL ON TABLE "public"."yo_perfil" TO "anon";
GRANT ALL ON TABLE "public"."yo_perfil" TO "authenticated";
GRANT ALL ON TABLE "public"."yo_perfil" TO "service_role";









ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "public" GRANT ALL ON SEQUENCES TO "postgres";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "public" GRANT ALL ON SEQUENCES TO "anon";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "public" GRANT ALL ON SEQUENCES TO "authenticated";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "public" GRANT ALL ON SEQUENCES TO "service_role";






ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "public" GRANT ALL ON FUNCTIONS TO "postgres";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "public" GRANT ALL ON FUNCTIONS TO "anon";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "public" GRANT ALL ON FUNCTIONS TO "authenticated";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "public" GRANT ALL ON FUNCTIONS TO "service_role";






ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "public" GRANT ALL ON TABLES TO "postgres";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "public" GRANT ALL ON TABLES TO "anon";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "public" GRANT ALL ON TABLES TO "authenticated";
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" IN SCHEMA "public" GRANT ALL ON TABLES TO "service_role";































