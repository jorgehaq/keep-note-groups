...
BEGIN
    IF (NEW.title IS DISTINCT FROM OLD.title) THEN
        -- Sincronizar hacia Note vinculada
        IF (NEW.linked_note_id IS NOT NULL) THEN
            UPDATE public.notes SET title = NEW.title WHERE id = NEW.linked_note_id AND (title IS DISTINCT FROM NEW.title);
        END IF;

        -- Sincronizar hacia Pizarrón vinculado
        IF (NEW.linked_board_id IS NOT NULL) THEN
            UPDATE public.brain_dumps SET title = NEW.title WHERE id = NEW.linked_board_id AND (title IS DISTINCT FROM NEW.title);
        END IF;
    END IF;
    RETURN NEW;
END;
